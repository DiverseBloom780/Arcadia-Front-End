...
namespace Arcadia.Core.Services
{
    ...
    public class GameLauncher
    {
        ...
        public bool LaunchGame(Game game)
        {
            ...
            try
            {
                launched = game.LaunchType switch
                {
                    LaunchType.Emulator => LaunchEmulator(game),
                    LaunchType.Steam => LaunchSteamGame(game),
                    LaunchType.GOG => LaunchGOGGame(game),
                    LaunchType.EpicGames => LaunchEpicGame(game),
                    LaunchType.TeknoParrot => LaunchTeknoParrotGame(game),
                    LaunchType.Standalone => LaunchStandaloneGame(game),
                    _ => throw new NotSupportedException($"Launch type {game.LaunchType} is not supported")
                };
                return launched;
            }
            catch (Exception ex)
