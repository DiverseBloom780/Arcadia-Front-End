    ...
    public class EpicGamesIntegration
    {
        ...
        private Game? ParseEpicManifest(string manifestPath)
        {
            try
            {
                ...
                if (LauncherUtils.IsNonGameApplication(displayName, installLocation))
                    return null;
                ...
                if (!string.IsNullOrEmpty(installLocation) && !string.IsNullOrEmpty(launchExecutable))
                {
                    ...
                    else if (Directory.Exists(fullExePath))
                    {
                        game.ExecutablePath = LauncherUtils.FindBestExecutable(fullExePath, displayName);
                    }
                }
                else if (!string.IsNullOrEmpty(installLocation))
                {
                    game.ExecutablePath = LauncherUtils.FindBestExecutable(installLocation, displayName);
                }
