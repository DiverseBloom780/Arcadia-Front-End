...
namespace Arcadia.UI.Rendering
{
    ...
    public class WheelRenderer
    {
        ...
        private void DrawWheelItem(DeviceContext context, float relativeIndex, ShaderResourceView? texture)
        {
            ...
            // Build 3D Transform
            System.Numerics.Matrix4x4 transform = System.Numerics.Matrix4x4.CreateScale(scale * LogoWidth, scale * LogoHeight, 1f);
            
            if (CurrentMode == WheelMode.Angled)
            {
                // Apply 3D rotation for the "premium" curved-away look
                transform *= System.Numerics.Matrix4x4.CreateRotationY(TiltAngle * (1.0f - scale));
            }
            
            transform *= System.Numerics.Matrix4x4.CreateTranslation(x, y, 0);
            ...
            var cbData = new PerObjectBuffer
            {
                ...
                ColorTint = isSelected ? new SharpDX.Vector4(1.2f, 1.2f, 1.2f, 1f) : new SharpDX.Vector4(AccentColor.Red, AccentColor.Green, AccentColor.Blue, AccentColor.Alpha)
            };
